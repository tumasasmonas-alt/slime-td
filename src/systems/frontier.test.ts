...
describe('computeFrontier / nearestFrontierPoint', () => {
  ...
  it('never reports growth inside the safe radius as reachable', () => {
    const state = freshState();
    state.grid = makeTestGrid();
    state.tower.x = 150;
    state.tower.y = 150;
    // Reveal a cell inside the safe radius — the ray-cast starts at
    // safeRadius, so this should never be found.
    const cx = Math.floor(155 / state.grid.cellSize);
    const cy = Math.floor(150 / state.grid.cellSize);
    const idx = cy * state.grid.cols + cx;
    state.grid.threshold[idx] = 0.1;
    state.grid.growth[idx] = 0.9;
