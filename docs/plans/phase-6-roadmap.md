...
| **▶ THE GATE** | ✅ **Ran and passed, 2026-08-10** (Decision 87). *Moved a third time (Decision 81) — ran after 6C, not after 6B.* Both verdicts: enhancement is **a decision, not a slider** — the socketing loop is closed as sound; and the **65-gem count is a go, unchanged**, so 6D ships its full 19. The bugs the playtest found are Decision 86, fixed and shipped in the same pass. | Ten weapons against three deck slots is a stronger place to ask this than the three-weapon deck 6B alone left behind. Build order was unaffected, as every previous move also was. |
| **6D** | ⚠️ **Split into four sub-batches on 2026-08-10** after reading the tuning constants against shipped code found four structural defects the design docs didn't show. **6D-0/6D-1/6D-2 shipped 2026-08-11** (Decisions 88–91); **6D-3 partially shipped 2026-08-11** (Steps 1–3, Decision 92) — **Steps 4–5 remain unbuilt.** ① **6D-0** — a balance-shape pass that was never in this table at all: unbounded ambient/event escalation, the **aura reach fix**, weapon spread, bounded time-scaled armour. ② **6D-1** — Targeting, **7 not 8** (Scattershot cut). ③ **6D-2** — Conditional, **9 not 11** (Shatter/Sterilizer cut as duplicates of shipped 6B extensions). ④ **6D-3** — the gem-reality fix, making six already-shipped gems do what their cards claim. Steps 1–3 (wiring, `clearAt`'s `ClearResult`, per-archetype Fork/Chaining/Bounce/Ricochet readings) shipped; Step 4 (the Multishot/Formation emission-multiplication rule) and Step 5 (copy fix, test matrix) are still open. | **This row's original estimate — "14 free, 5 modifier, 0 new" — was wrong**, and the way it was wrong is the useful part: it is a *rendering* cost table, and rendering was never this batch's cost. The real costs were a difficulty curve that plateaus while player power doesn't (6D-0), an ACQUIRE stage four weapons don't have for Targeting gems to replace (6D-1), and six shipped gems wired into one weapon of ten (6D-3 — Fork/Chaining/Bounce/Ricochet's half fixed in Steps 1–3, Multishot/Formation's half still open in Step 4). Step 4 also needs genuinely new rendering (satellite orbits, concentric rings) — the part of 6D-3 not yet built. Full account: `docs/sessions/2026-08-10-phase-5-gate-and-6d-planning.md` (the findings) and `docs/sessions/2026-08-11-phase-6d-batches.md` (the build). |
