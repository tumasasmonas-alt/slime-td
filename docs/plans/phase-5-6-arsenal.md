...
| **▶ small check** | *(after 5C)* Does the inventory screen open, does the `+/-` read legibly, does anything break? Answerable immediately. **The full gate moved** — see below. | |
...
| **▶ THE GATE** | **Moved here from after 5C on 2026-08-08.** Playtest the socketing loop, judge the gem count, and answer *is enhancement a decision or a slider?* — the question the whole socket ladder exists to settle. **It could not be asked before this point**: with sockets empty, opening a 4th socket buys nothing, so specialising has no benefit and the answer is forced to "slider" regardless of whether the design is sound. Asking it here is the first time the answer means anything. Build order was not changed to accommodate this; only the gate moved. See `docs/plans/phase-5c-inventory-ui.md` §1. | |
